...
// import { categorie_routes_items } from "../../routes/categorie/CategoriesRoute";
...
export default function ClientList() {
    ...
    const deleteItems = async (itemsIds) => {
        try {
            ...
            await fetchApi(`/administration/client/${itemsIds}`, {
                method: 'DELETE',
            })

            dispacth(setToastAction({ severity: 'success', summary: 'Client supprimé', detail: "Le client a été supprimé avec succès", life: 3000 }))
            fetchCategories()
            setSelectAll(false)
        ...
        confirmDialog({
            ...
            message: <div className="d-flex flex-column align-items-center">
                {inViewMenuItem ? <>
                    ...
                    <div className="font-bold text-center my-2">{inViewMenuItem?.NOM_CLIENT} {inViewMenuItem?.PRENOM_CLIENT}</div>
    ...
    const fetchClients = useCallback(async () => {
        try {
            setLoading(true)
            const baseurl = `/administration/client?`
        ...
        <>
            ...
            <div className="px-4 py-3 main_content">
                <div className="d-flex align-items-center justify-content-between">
                    <h1 className="mb-3">Clients</h1>
                    <Button label="Nouveau client" icon="pi pi-plus" size="small" onClick={() => {
                        navigate("/client/new")
                    }} />
                </div>
                ...
                <div className="content">
                    <div className="shadow rounded mt-3 pr-1 bg-white">
                        ...
                        >
                            ...
                            <Column field="NOM_CLIENT" frozen header="Nom" sortable body={item => {
                                return (
                                    <span>{item.NOM_CLIENT}</span>
                                )
                            }} />

                            <Column field="PRENOM_CLIENT" frozen header="Prenom" sortable body={item => {
                                return (
                                    <span>{item.PRENOM_CLIENT}</span>
                                )
                            }} />

                             <Column field="ADDRESSE_CLIENT" frozen header="Adresse complete" sortable body={item => {
                                return (
                                    <span>{item.ADDRESSE_CLIENT}</span>
                                )
                            }} />
                            <Column field="TEL_CLIENT" frozen header="Telephone" sortable body={item => {
                                return (
                                    <span>{item.TEL_CLIENT}</span>
                                )
                            }} />
                            <Column field="EMAIL_CLIENT" frozen header="Adresse e-mail" sortable body={item => {
                                return (
                                    <span>{item.EMAIL_CLIENT}</span>
                                )
                            }} />
                                ...
                                const items = [
                                   {
                                        template: () => {
                                            return (
                                                <Link to={`/client/edit/${inViewMenuItem?.ID_CLIENT }`} className="p-menuitem-link">
                                    ...
                                    },
                                    {
                                        template: () => {
                                            return (
                                                <a href="#" className="p-menuitem-link text-danger" onClick={e => handleDeletePress(e, [inViewMenuItem.ID_CLIENT ])}>
